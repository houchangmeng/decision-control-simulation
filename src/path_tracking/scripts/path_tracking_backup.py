...
class PathTracking:
    ...
    def calculateTwistCommand(self):
        ...
        currentSpeed = math.sqrt(math.pow(currentSpeedX, 2) + math.pow(currentSpeedY,2)) / 3.6# km/h -> m/s
        # currentSpeed = currentSpeedX / 3.6
        horizon = self.dt * currentSpeed
        ...
        currentTimeIndex = int(np.ceil( (rospy.Time.now() - self.startTime).to_sec()/30 * 100 )) + 1
        currentTimeIndex = min(currentTimeIndex, 99)
        targetWaypoint = self.globalWaypoints.waypoints[currentTimeIndex]
    
        targetX = targetWaypoint.pose.pose.position.x
        targetY = targetWaypoint.pose.pose.position.y
        targetQuanternion = (targetWaypoint.pose.pose.orientation.x, targetWaypoint.pose.pose.orientation.y, targetWaypoint.pose.pose.orientation.z,targetWaypoint.pose.pose.orientation.w)
        euler = tf.transformations.euler_from_quaternion(targetQuanternion)
        targetYaw = euler[2]
        targetSpeedX = targetWaypoint.twist.twist.linear.x
        targetSpeedY = targetWaypoint.twist.twist.linear.y
        ...
        dervTargetSpeed = (targetX - self.lastTargetX) * (100.0 / 30.0)
        self.lastCurrentX = currentX
        self.lastTargetX = targetX
        self.target_vel.append(dervTargetSpeed) # blue
        self.tracking_vel.append(dervCurrentSpeed) # orange
        ...
        if (currentTimeIndex < (len(self.globalWaypoints.waypoints)) - 2):
            twistCmd = Twist()
            twistCmd.linear.x = speed
            twistCmd.angular.z = theta
            finish = False
            # print("targetIndex",targetIndex)
            # print("---command speed",speed)
...
class Incremental_PID(object):
    ...
    def run_step(self,targetWaypoint2D, currentWaypoint2D):
        ...
        positionIncrement = 4.0* (es - self.lastPositionError) + 0.00 * es + 0.00 * (es - 2*self.lastPositionError + self.lastLastPositionError)
        ...
        velocityIncrement = 0.2 * (ev - self.lastVelocityError) + 0.00 * es + 0.00 * (es - 2*self.lastVelocityError + self.lastLastVelocityError)
        self.pidVelocityPutput +=velocityIncrement
        pid_velocity_output = np.clip(self.pidVelocityPutput, -2.0, 2.0)
